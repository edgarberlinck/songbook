...
function App() {
  ...
  const filteredSongs = useMemo(() => {
    return [...library.songs]
      ...
      .filter((song) => selectedTag === 'all' || song.tags.includes(selectedTag))
      .filter((song) => {
        const haystack = [song.title, song.artist ?? '', song.key ?? '', song.tags.join(' ')].join(' ').toLowerCase()
  ...
  async function handleSave() {
    ...
    const song = await invoke<SongDetail>('save_song', { id: activeSong.id, content: draft })
    setActiveSong(song)
    setPreviewSong(song)
    setDraft(song.content)
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      const preview = await invoke<SongDetail>('preview_song', { id: activeSong.id, content: draft })
    ...
    <main className={`app-shell ${readingMode ? 'reading-mode' : ''}`}>
      <aside className="sidebar">
        ...
        <div className="segmented">
          <button className={!showFavoritesOnly ? 'active' : ''} onClick={() => setShowFavoritesOnly(false)}>All Songs</button>
          <button className={showFavoritesOnly ? 'active' : ''} onClick={() => setShowFavoritesOnly(true)}>Favorites</button>
        </div>

        <label className="search">
          <span>Search</span>
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Title, artist, key, tag" />
        </label>

        <div className="controls-row">
          <label>
            <span>Sort</span>
            <select value={sortBy} onChange={(event) => setSortBy(event.target.value as keyof typeof sorters)}>
        ...
        <section className="library-section">
          ...
          <ul className="song-list">
            {filteredSongs.map((song) => {
              ...
              return (
                <li key={song.id}>
                  <button className={`song-card ${active ? 'active' : ''}`} onClick={() => void selectSong(song.id)}>
                    <input
                      type="checkbox"
                      checked={selected}
                      onChange={(event) => {
                        event.stopPropagation()
                        setSelectedIds((current) => event.target.checked ? [...new Set([...current, song.id])] : current.filter((item) => item !== song.id))
                      }}
                    />
                    <div>
                    ...
                    <small>{song.key ?? '—'} · {formatDate(song.lastModified)}</small>
                  </button>
                </li>
              )
            })}
      ...
      <section className="workspace">
        <header className="toolbar">
          <div>
            ...
            <p>{activeSong?.artist ?? 'Distraction-free writing and rehearsal'}</p>
          </div>
          <div className="toolbar-actions">
            <button onClick={() => void handleTranspose(-1)}>−1</button>
            <button onClick={() => void handleTranspose(1)}>+1</button>
            <button onClick={() => setShowChords((value) => !value)}>{showChords ? 'Hide chords' : 'Show chords'}</button>
            <button onClick={() => setReadingMode((value) => !value)}>{readingMode ? 'Editor mode' : 'Reading mode'}</button>
            <button onClick={() => setTheme((value) => value === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? 'Light' : 'Dark'} mode</button>
        ...
        <div className="split-view">
          ...
          <section className="pane preview-pane" style={{ fontSize: `${fontScale}rem` }}>
              ...
              {displayedSong && (
                <article>
                  <div className="preview-heading">
                    ...
                    {displayedSong.subtitle ? <p>{displayedSong.subtitle}</p> : null}
                    <div className="chips">
                      {displayedSong.key ? <span>Key {displayedSong.key}</span> : null}
                      {displayedSong.capo !== null ? <span>Capo {displayedSong.capo}</span> : null}
                      {displayedSong.favorite ? <span>Favorite</span> : null}
                    </div>
                  </div>

                  {preview.map((line, index) => (
                    <div key={`${index}-${line.label ?? line.lyricLine ?? 'line'}`} className={`rendered-line ${line.kind} ${line.chorus ? 'chorus' : ''}`}>
                      {line.kind === 'section' ? <h3>{line.label}</h3> : null}
                      {line.kind === 'meta' ? <p className="meta-line">{line.label}</p> : null}
                      {line.kind === 'lyric' ? (
                        <>
                          {showChords && line.chordLine ? <pre className="chord-line">{line.chordLine}</pre> : null}
                          <pre className="lyric-line">{line.lyricLine}</pre>
                        </>
                      ) : null}
                    </div>
                  ))}
